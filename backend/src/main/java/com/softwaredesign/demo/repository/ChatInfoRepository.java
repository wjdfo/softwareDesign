...
import com.softwaredesign.demo.dto.ChatListDto;
...
public interface ChatInfoRepository extends JpaRepository<ChatInfo, Long> {
    ...
    @Query("select new com.softwaredesign.demo.dto.ChatListDto(c.chat_id, a.article_id, a.title) "
        + "from Article a, ChatInfo c "
        + "where a.article_id = c.article_id and (c.guest_id = :member_id or c.owner_id = :member_id)")
    List<ChatListDto> findChatInfoByMemberId(@Param("member_id") String member_id);
}
